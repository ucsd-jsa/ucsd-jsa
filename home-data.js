...
function getEvents() {
  var events = database.ref("events").orderByChild("date");
  events.once("value")
      ...
      snapshot.forEach(function (eventSnapshot) {
        if (count < maxCount) {
          ...
          if (eventSnapshot.child("fbLink").val() !== "" && currentDate.isBefore(date)) {
            ...
            document.getElementById("event-placeholders").classList.add("hidden");
...
function getHomeData() {
  ...
  data.once("value")
    .then(function(snapshot) {
      ...
      // Language Table link
      var ltLink = snapshot.child("ltLink").val()
      document.getElementById("ltLink").setAttribute("href", ltLink);

      // JSA Hangouts link
      var hangoutsLink = snapshot.child("hangoutsLink").val()
      document.getElementById("hangoutsLink").setAttribute("href", hangoutsLink);

      // Home banner
      storage.refFromURL(snapshot.child("homeBanner").val()).getDownloadURL().then(function(url) {
        const banner = document.getElementById("banner");
        banner.style.backgroundImage = "url(" + url + ")";
        banner.classList.add("animated", "fadeIn", "slow");

        const logo = document.getElementById("logo");
        logo.classList.add("animated", "fadeIn", "slow", "visible");
        logo.classList.remove("invisible");

        const bannerText = document.getElementById("banner-text");
        bannerText.classList.add("animated", "fadeIn", "slow", "delay-1s", "visible");
        bannerText.classList.remove("invisible");

        const arrow = document.getElementById("arrow");
        arrow.classList.add("animated", "fadeInDown", "slow", "delay-2s", "visible");
        arrow.classList.remove("invisible");

        // All events image
        storage.refFromURL(snapshot.child("allEvents").val()).getDownloadURL().then(function (url) {
          document.getElementById("allEvents").setAttribute("href", url);
        });
      });
    });
}
